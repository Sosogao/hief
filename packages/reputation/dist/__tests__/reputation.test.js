...
function makeMetrics(overrides = {}) {
    return {
        ...scoringEngine_1.ScoringEngine.emptyMetrics(ADDR_A, CHAIN_BASE),
...
describe('ScoringEngine', () => {
    ...
    test('new address with no activity has very low final score (decay applied)', () => {
        const metrics = scoringEngine_1.ScoringEngine.emptyMetrics(ADDR_B, CHAIN_BASE);
        const scores = engine.computeScores(metrics);
    ...
    test('applyIntentEvent increments metrics correctly', () => {
        const metrics = scoringEngine_1.ScoringEngine.emptyMetrics(ADDR_A, CHAIN_BASE);
...
describe('Reputation API', () => {
    beforeAll(async () => {
        await (0, server_1.initServer)();
    });
    test('GET /v1/reputation/health returns ok', async () => {
        const res = await (0, supertest_1.default)(server_1.app).get('/v1/reputation/health');
        expect(res.status).toBe(200);
        expect(res.body.status).toBe('ok');
    });
    test('GET /v1/reputation/:chainId/:address returns snapshot', async () => {
        const res = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/${ADDR_A}`);
        expect(res.status).toBe(200);
    ...
    test('GET /v1/reputation/:chainId/:address returns 400 for invalid address', async () => {
        const res = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/not-an-address`);
        expect(res.status).toBe(400);
    });
    test('GET /v1/reputation/:chainId/:address/history returns empty array for new address', async () => {
        const res = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/${ADDR_B}/history`);
        expect(res.status).toBe(200);
    ...
    test('GET /v1/reputation/:chainId/leaderboard returns array', async () => {
        const res = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/leaderboard`);
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body.data)).toBe(true);
    });
    test('score increases after successful intent event', async () => {
        // Get initial score
        const before = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/${ADDR_B}`);
        ...
        const after = await (0, supertest_1.default)(server_1.app).get(`/v1/reputation/${CHAIN_BASE}/${ADDR_B}?refresh=true`);
