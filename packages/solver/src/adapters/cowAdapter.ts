...
const COW_SETTLEMENT: Record<number, string> = {
...
const COW_VAULT_RELAYER: Record<number, string> = {
 ...
 */
export async function getCowQuote(intent: HIEFIntent): Promise<CowQuote | null> {
 ...
 */
export function buildSolutionFromCowQuote(
  intent: HIEFIntent,
  quote: CowQuote,
  solverId: string
): HIEFSolution {
  ...
  const cowSettlementIface = new ethers.Interface([
  ...
  const orderUid = ethers.hexlify(ethers.randomBytes(56)); // 56 bytes = orderUid format
  ...
  const intentHash = ethers.hexlify(ethers.randomBytes(32)); // Will be computed properly in production
