...
const safeAdapter_1 = require("../adapters/safeAdapter");
...
function makeCowQuote(overrides = {}) {
    return {
...
describe('CoW Adapter', () => {
    it('should build a valid HIEF solution from a CoW quote', () => {
        ...
        const solution = (0, cowAdapter_1.buildSolutionFromCowQuote)(intent, quote, wallet.address);
...
describe('Safe Adapter', () => {
    it('should build a Safe transaction from a solution', async () => {
        ...
        const quote = makeCowQuote();
        const solution = (0, cowAdapter_1.buildSolutionFromCowQuote)(intent, quote, wallet.address);
        solution.intentHash = ethers_1.ethers.hexlify(ethers_1.ethers.randomBytes(32));
        ...
        const result = await (0, safeAdapter_1.buildSafeTransaction)(intent, solution, mockPolicyResult, wallet.address, // using wallet as mock safe address
        8453);
    ...
    it('should use MultiSend for multiple calls', async () => {
        ...
        const result = await (0, safeAdapter_1.buildSafeTransaction)(intent, solution, mockPolicyResult, wallet.address, 8453);
