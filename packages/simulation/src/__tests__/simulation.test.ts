...
function makeMockSimResponse(overrides: Partial<{
  status: boolean;
  errorMessage: string;
  gasUsed: number;
  assetChanges: any[];
...
function makeSwapSolution(overrides: Partial<{
  slippageBps: number;
  quoteUsd: number;
  outputToken: string;
  operation: number;
...
describe('calcNetOutflowUsd', () => {
  test('calculates net outflow for user address', () => {
    ...
    const netOutflow = calcNetOutflowUsd(diff, USER_ADDR);
...
describe('findUnlimitedApprovals', () => {
  test('returns only unlimited approvals for user', () => {
    ...
    const unlimited = findUnlimitedApprovals(diff, USER_ADDR);
...
describe('SimulationEngine', () => {
  ...
  function mockTenderly(response: TenderlySimulationResponse): TenderlyClient {
    const client = Object.create(TenderlyClient.prototype) as TenderlyClient;
    (client as any).simulate = jest.fn().mockResolvedValue(response);
    (client as any).simulateBundle = jest.fn().mockResolvedValue({
  ...
  test('SIM-00: returns SKIP when no Tenderly client', async () => {
    const engine = new SimulationEngine(null);
  ...
  test('SIM-01: FAIL when simulation reverts', async () => {
    ...
    const engine = new SimulationEngine(mockTenderly(resp));
    const result = await engine.verify(makeSwapSolution());
  ...
  test('returns SKIP when Tenderly returns null (unreachable)', async () => {
    const client = Object.create(TenderlyClient.prototype) as TenderlyClient;
    (client as any).simulate = jest.fn().mockResolvedValue(null);
    const engine = new SimulationEngine(client);
    const result = await engine.verify(makeSwapSolution());
    expect(result.status).toBe('SKIP');
  });
});
