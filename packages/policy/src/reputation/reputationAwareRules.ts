...
import { runStaticRules, RuleResult } from '../rules/staticRules';
 ...
 */
export function runReputationAwareRules(
  intent: HIEFIntent,
  solution: HIEFSolution,
  params: DynamicPolicyParams
): ReputationAwareRuleResult {
  const reputationFindings: PolicyFinding[] = [];

  // ── Run base static rules ────────────────────────────────────────────────
  const { results, hasCriticalFailure } = runStaticRules(intent, solution);
  ...
  const hasHigh =
    allResults.some((r) => !r.passed && HIGH_RULES.has(r.ruleId)) ||
