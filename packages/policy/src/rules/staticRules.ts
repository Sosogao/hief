...
// Internal finding type that includes 'field' for debugging
...
const R1_deadline: RuleFn = (intent) => {
...
// ─── R2: intentId Binding ─────────────────────────────────────────────────
const R2_intentBinding: RuleFn = (intent, solution) => {
...
const R3_quoteExpiry: RuleFn = (_intent, solution) => {
  const now = Math.floor(Date.now() / 1000);
...
// ─── R4: Fee Within Limit ──────────────────────────────────────────────────
const R4_feeCap: RuleFn = (_intent, solution) => {
  ...
  const passed = feeBps <= BigInt(POLICY.MAX_FEE_BPS);
  return {
    ruleId: 'R4',
    passed,
    severity: 'HIGH',
    finding: passed ? undefined : {
      ruleId: 'R4',
      severity: 'HIGH',
      message: `Fee ${feeBps}bps exceeds max ${POLICY.MAX_FEE_BPS}bps`,
      field: 'solution.quote.fee',
      actual: feeBps.toString(),
      expected: `<= ${POLICY.MAX_FEE_BPS}`,
    },
  };
};

// ─── R5: Slippage Within Limit ─────────────────────────────────────────────
const R5_slippageCap: RuleFn = (intent) => {
  const slippage = intent.constraints.slippageBps ?? 0;
  const passed = slippage <= POLICY.MAX_SLIPPAGE_BPS;
  return {
    ruleId: 'R5',
    passed,
    severity: 'HIGH',
    finding: passed ? undefined : {
      ruleId: 'R5',
      severity: 'HIGH',
      message: `Slippage ${slippage}bps exceeds max ${POLICY.MAX_SLIPPAGE_BPS}bps`,
...
const R6_noBlacklistedSelectors: RuleFn = (_intent, solution) => {
  const violations: string[] = [];
  for (const call of solution.executionPlan.calls) {
    if (call.data && call.data.length >= 10) {
      const selector = call.data.slice(0, 10).toLowerCase();
      if (BLACKLISTED_SELECTORS.has(selector)) {
...
const R7_noUnlimitedApproval: RuleFn = (_intent, solution) => {
  const APPROVE_SELECTOR = '0x095ea7b3';
  const MAX_UINT256 = BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff');
  const violations: string[] = [];

  for (const call of solution.executionPlan.calls) {
    if (!call.data || call.data.length < 10) continue;
    const selector = call.data.slice(0, 10).toLowerCase();
...
const R8_protocolWhitelist: RuleFn = (_intent, solution) => {
  const unknownTargets: string[] = [];
  for (const call of solution.executionPlan.calls) {
    const addr = call.to.toLowerCase();
    if (!WHITELISTED_PROTOCOLS.has(addr) && !WHITELISTED_PROTOCOLS.has(call.to)) {
...
const R10_outputDefined: RuleFn = (intent) => {
...
// ─── R11: No DELEGATECALL ─────────────────────────────────────────────────
const R11_noDelegatecall: RuleFn = (_intent, solution) => {
...
const R12_chainId: RuleFn = (intent) => {
...
export const CRITICAL_RULES = new Set(['R1', 'R2', 'R6', 'R10', 'R11', 'R12']);
export const HIGH_RULES = new Set(['R3', 'R4', 'R5', 'R7']);

export function runStaticRules(
  intent: HIEFIntent,
  solution: HIEFSolution
): { results: RuleResult[]; hasCriticalFailure: boolean; hasHighFailure: boolean } {
  const results = STATIC_RULES.map((rule) => rule(intent, solution));
  const hasCriticalFailure = results.some((r) => !r.passed && CRITICAL_RULES.has(r.ruleId));
  const hasHighFailure = results.some((r) => !r.passed && HIGH_RULES.has(r.ruleId));
