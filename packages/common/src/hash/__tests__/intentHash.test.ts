...
import { computeIntentHash, verifyIntentSignature } from '../intentHash';
import { computeSolutionHash, computePlanHash } from '../solutionHash';
import type { HIEFIntent, HIEFSolution } from '../../types';
...
function makeTestIntent(overrides: Partial<HIEFIntent> = {}): HIEFIntent {
  const base: HIEFIntent = {
    intentVersion: '0.1',
...
describe('computeSolutionHash', () => {
  function makeTestSolution(intentHash: string): HIEFSolution {
    return {
      solutionVersion: '0.1',
  ...
  it('should produce a deterministic solution hash', () => {
    ...
    const h1 = computeSolutionHash(solution);
    const h2 = computeSolutionHash(solution);
    expect(h1).toBe(h2);
  ...
  it('planHash should bind solution to intentHash', () => {
    ...
    const intentHash = computeIntentHash(intent);
    const solution = makeTestSolution(intentHash);
    const planHash = computePlanHash(solution, intentHash);
    expect(planHash).toMatch(/^0x[0-9a-f]{64}$/i);

    // Different intentHash should produce different planHash
    const otherHash = computeIntentHash(makeTestIntent({ deadline: 1 }));
    const otherPlanHash = computePlanHash(solution, otherHash);
