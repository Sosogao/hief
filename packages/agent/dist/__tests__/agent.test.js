...
describe('TokenRegistry', () => {
    test('resolves USDC on Base by symbol', () => {
        const token = (0, tokenRegistry_1.resolveToken)('USDC', BASE_CHAIN_ID);
    ...
    test('resolves ETH on Base by alias "以太"', () => {
        const token = (0, tokenRegistry_1.resolveToken)('以太', BASE_CHAIN_ID);
        expect(token).not.toBeNull();
        expect(token.symbol).toBe('ETH');
    });
    test('resolves USDC by alias "u"', () => {
        const token = (0, tokenRegistry_1.resolveToken)('u', BASE_CHAIN_ID);
        expect(token).not.toBeNull();
        expect(token.symbol).toBe('USDC');
    });
    test('resolves BTC alias "比特币" to cbBTC on Base', () => {
        const token = (0, tokenRegistry_1.resolveToken)('比特币', BASE_CHAIN_ID);
        expect(token).not.toBeNull();
        expect(token.symbol).toBe('cbBTC');
    });
    test('resolves token by direct address', () => {
        const token = (0, tokenRegistry_1.resolveToken)('0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', BASE_CHAIN_ID);
        expect(token).not.toBeNull();
        expect(token.symbol).toBe('USDC');
    });
    test('returns null for unknown token', () => {
        const token = (0, tokenRegistry_1.resolveToken)('UNKNOWN_XYZ', BASE_CHAIN_ID);
        expect(token).toBeNull();
    });
    test('parseAmount converts 100 USDC correctly (6 decimals)', () => {
        const raw = (0, tokenRegistry_1.parseAmount)('100', 6);
        expect(raw).toBe('100000000');
    });
    test('parseAmount converts 0.5 ETH correctly (18 decimals)', () => {
        const raw = (0, tokenRegistry_1.parseAmount)('0.5', 18);
        expect(raw).toBe('500000000000000000');
    });
    test('formatAmount converts raw USDC back to human', () => {
        const human = (0, tokenRegistry_1.formatAmount)('100000000', 6);
        expect(human).toBe('100');
    });
    test('getChainName returns correct names', () => {
        expect((0, tokenRegistry_1.getChainName)(8453)).toBe('Base');
        expect((0, tokenRegistry_1.getChainName)(1)).toBe('Ethereum');
        expect((0, tokenRegistry_1.getChainName)(84532)).toBe('Base Sepolia');
    });
});
// ─── IntentParser Tests ────────────────────────────────────────────────────────
describe('IntentParser', () => {
    ...
    beforeEach(() => {
        ...
        parser = new intentParser_1.IntentParser({ apiKey: 'test-key' });
        mockCreate = parser.client.chat.completions.create;
    });
    ...
    test('parseAndResolve builds valid HIEFIntent for SWAP', async () => {
        ...
        const resolved = await parser.parseAndResolve('swap 100 USDC to ETH', MOCK_SMART_ACCOUNT, BASE_CHAIN_ID);
        ...
        expect(resolved.hief.meta?.uiHints?.inputTokenSymbol).toBe('USDC');
        expect(resolved.hief.meta?.uiHints?.outputTokenSymbol).toBe('ETH');
    });
    test('parseAndResolve returns error for unknown token', async () => {
        ...
        const resolved = await parser.parseAndResolve('swap 100 UNKNOWN_XYZ to ETH', MOCK_SMART_ACCOUNT, BASE_CHAIN_ID);
    ...
    test('parseAndResolve returns unsupported error for BRIDGE', async () => {
        ...
        const resolved = await parser.parseAndResolve('bridge 1 ETH to Arbitrum', MOCK_SMART_ACCOUNT, BASE_CHAIN_ID);
        expect(resolved.hief).toBeUndefined();
...
describe('ConversationEngine', () => {
    ...
    beforeEach(() => {
        const OpenAI = require('openai').default;
        engine = new conversationEngine_1.ConversationEngine({ apiKey: 'test-key' });
        // Access the internal parser's mock
        mockCreate = engine.client.chat.completions.create;
        // Also mock the parser's client
        engine.parser.client = engine.client;
    });
